...
**Combined `review` + `rewrite`.** A requested word ceiling is the budget for the whole response, not for each part. Review only the highest-impact distinct defects, rewrite once, and name cut categories once. Do not explain the same defect again in the cut report after quoting it in the review.
